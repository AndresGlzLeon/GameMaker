/// @description Dibujar Pausa y detectar clics de botones
...
// --- 1. BOTÓN "MENU" (Cuando NO hay pausa) ---
if (!pausa) {
    draw_set_font(fnt_Botones);
    var hover_btn = point_in_rectangle(mx, my, btn_x, btn_y, btn_x + btn_ancho, btn_y + btn_alto);
    ...
    if (hover_btn && click) {
        ...
        show_debug_message("⏸ PAUSA ACTIVADA");
    }
...
// --- 2. PANTALLA DE PAUSA (Cuando SÍ hay pausa) ---
if (pausa) {
    var cx = ancho / 2;
    var cy = alto / 2;
    ...
    var b1_y1 = cy - 60;  var b1_y2 = cy - 10;  // CONTINUAR
    var b2_y1 = cy;       var b2_y2 = cy + 50;  // GUARDAR
    var b3_y1 = cy + 60;  var b3_y2 = cy + 110; // SALIR
    
    var bx1 = cx - 120; var bx2 = cx + 120;
    ...
    if (h1 && click) {
        ...
        show_debug_message("▶ PAUSA DESACTIVADA");
    }
    ...
    if (h2 && click) {
        show_debug_message("💾 GUARDANDO PARTIDA...");
    ...
    if (h3 && click) {
        show_debug_message("🚪 SALIENDO AL MENÚ...");
        instance_activate_all();
        room_goto(rm_Menu);
    }
